...
VAR_MARIADB_DOCK_IMG="$CFG_MKLIVE_MARIADB_DOCKERIMAGE"
VAR_MARIADB_DB_DIR_TAR_FN="mariadb-dbs-vanilla.tgz"

VAR_NGINX_DOCK_IMG="$CFG_MKLIVE_NGINX_DOCKERIMAGE"
...
if [ $VAR_EXITCODE -eq 0 -a "$VAR_TRAPPED_INT" != "true" ]; then
	...
	_mklive_replaceVarInDcTempl "MDB_LIVE_DOCKERIMAGE" "$VAR_MDB_DOCK_IMG_LIVE_OUTPUT"
	...
	# Nginx
	_mklive_replaceVarInDcTempl "MDB_NGINX_DOCKERIMAGE" "$VAR_NGINX_DOCK_IMG"

	# DB-Server
	_mklive_replaceVarInDcTempl "MDB_MARIADB_DOCKERIMAGE" "$VAR_MARIADB_DOCK_IMG"
